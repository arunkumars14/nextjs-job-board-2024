...
function RecruiterJobCard({jobItem, jobApplications}) {
    ...
    const [showCurrentCandidateDetailModel, setShowCurrentCandidateDetailModel] = useState(false)
    ...
    return (
        <div>
            <CommonCard 
                ...
                footerContent={
                    <Button onClick={()=> setShowApplicantsDrawer(true)} className="disabled:opacity-70 flex h-11 items-center justify-center px-5" disabled={jobApplications.filter(item => item.jobID === jobItem?._id).length === 0}>
                        {
